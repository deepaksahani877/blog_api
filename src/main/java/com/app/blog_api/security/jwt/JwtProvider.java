...
public class JwtProvider {
    ...
    public boolean validateJwtToken(String authToken) {
        try {
            ...
            JwtToken token = jwtTokenService.getToken(authToken);
            if(token.getIsExpired()||token.getIsRevoked()){
                logger.error("Invalid JWT token: {}", "Token expired or revoked");
                return false;
            }
            return true;
    ...
    public void invalidateUser(Long userId){
            List<JwtToken> previousTokens = jwtTokenService.getTokensByUserId(userId);
            List<JwtToken> updatePreviewsTokens = previousTokens.stream().peek(
                    (token)-> {
            ...
            jwtTokenService.createOrUpdateAllTokens(updatePreviewsTokens);
    }
}